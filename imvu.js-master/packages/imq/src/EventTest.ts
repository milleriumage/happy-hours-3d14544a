...
const handleEvent = <K extends Events['event']>(
   ...
   */
//[FIX] Use a type assertion to inform TypeScript that the handler and event types are compatible.
  return (handler as (event: Events) => Events)(event); // We know that this should work, but TS is not smart enough to figure it out?
};
