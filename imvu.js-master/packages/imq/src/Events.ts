...
const responses: EventHandlers<GatewayEvent, 'record'> = {
  ...
  msg_g2c_send_message: (data) => {
    return {
      ...data,
//[FIX] Replace Buffer with atob for browser compatibility
...
//[FIX] Use a type assertion to handle the union of function signatures.
  return (handler as (data: any) => any)(data);
}

export const encodeIMQComponent = (value: string | number | boolean): string => {
//[FIX] Replace Buffer with btoa for browser compatibility
...
//[FIX] Use a type assertion to handle the union of function signatures.
  return (messages[event] as (data: any) => any)(data);
}
