...
export class IMQManager {
	...
	private state: { status: typeof STATUS[keyof typeof STATUS]; connectAt?: number } = {
		status: 'disconnected',
		connectAt: Date.now(),
	};
	...
	private onConnectionState(status: Status, connectAt?: number) {
		this.state = {
//[FIX] Removed `?? 'unknown'` which caused a type error because 'unknown' is not a valid status.
