...
//[FIX] Correctly initialize the Map by ensuring the mapped array items are tuples and handling potentially undefined values. Add an explicit type to `avatars`.
		const avatars: Map<number, PartialProduct[]> = new Map(
			data.map((value: string): [number, PartialProduct[]] => {
